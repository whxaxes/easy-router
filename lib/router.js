...
rp.route = function (req, res) {
    ...
    for (; i < this.filters.length; i++) {
        ...
        if(ads[0] === "url"){
            ...
            if(isArray(filepath)){
                var html = '<html><meta charset="UTF-8">';
                html += '<meta content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0" name="viewport">';
                html += '<title>menu</title><body><style>a{display: block;padding:10px;font-size:18px;text-align: center;color:#999;}a:hover{color:#000;}</style>';
...
rp.getFolderList = function(folderpath , re){
    ...
    each(folders , function(f , i){
        ...
        try{
            ...
            if(fs.lstatSync(folderpath + path.sep + f).isDirectory()){
                f += "/";
            }else if(re.test(f)){
                p = RegExp.$1;
            }else return;
