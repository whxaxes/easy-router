...
rp.listen = function(port, callback) {
  ...
  server.on('listening', () => {
    ...
    !this.silent && logger.log('服务启动，监听' + logger.color(port, 'green') + '端口中...');
  ...
  _.each(maps, function(map, k) {
    if (_.isString(map)) {
      ...
      if (map.indexOf(':') >= 0) {
        ...
        // 把 xx/xx 转换成 ./xx/xx
        map = map.charAt(0) !== path.sep ? './' + map : map; 
        ad = ['url', map];
      }
...
rp.route = function(req, res) {
  ...
  !this.silent && logger.log(logger.color('404 get ' + reqUrl, 'gray'));
...
rp._processUrl = function(req, res, fil, ads, suitpath) {
  ...
  if (fs.lstatSync(filepath).isDirectory()) {
    if (this.debug) {
      ...
      !this.silent && logger.log(logger.color('200 get ' + suitpath, 'green'));
...
rp.routeTo = function(req, res, filepath, headers) {
  ...
  if (that.useCache && cacheAllow.indexOf(type) >= 0) {
    ...
    if (req.headers['if-none-match'] === etag) {
      that.cache(res);
      !this.silent && logger.log(logger.color('304 cache ' + filepath, 'yellow'));
      return true;
    }
  ...
  !this.silent && logger.log(logger.color('200 get ' + req.url, 'green'));
...
rp._parsePath = function(fil, ads, pathname) {
  ...
  if (AB_RE.test(fil) && !(AB_RE.lastIndex = 0) && AB_RE.test(ads)) {
    ...
    _.each(collector, function(c) {
      ...
      // 扫描路径，当遇到AB关键字时处理，如果两者不相等，
      // 停下adsArray的扫描，继续执行对filArray的扫描，
      // 直至遇到相等数值
